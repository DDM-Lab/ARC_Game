using System;
using UnityEngine;

namespace CityBuilderCore
{
    /// <summary>
    /// attempts to walk to a point
    /// </summary>
    [Serializable]
    public class WalkPointActionTyped : WalkerAction
    {
        [SerializeField]
        public Vector2Int _point;
        [SerializeField]
        public PathType _pathType;

        public WalkPointActionTyped()
        {

        }
        public WalkPointActionTyped(Vector2Int point, PathType pathType)
        {
            _point = point;
            _pathType = pathType;
        }

        public override void Start(Walker walker)
        {
            base.Start(walker);

            if (!walker.Walk(_point, _pathType, null, walker.AdvanceProcess))
                walker.AdvanceProcess();
        }
        public override void Continue(Walker walker)
        {
            base.Continue(walker);

            walker.ContinueWalk(walker.AdvanceProcess);
        }
        public override void Cancel(Walker walker)
        {
            base.Cancel(walker);

            walker.CancelWalk();
        }
    }
}
