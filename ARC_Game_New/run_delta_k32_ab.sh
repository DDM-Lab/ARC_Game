...
echo "### DELTA(K=32) A/B  compact cmd + --history 32  -> bench_format_ablation/delta_minimal_cmd_k32"
python3 benchmark_models.py --policy llm --models "$MODELS" \
  --image_mode none --action_format cmd --transfers manual \
  --system_prompt minimal --obs_encoding delta --history 32 --reasoning_effort low \
  --episodes 3 --rounds 32 --workers 8 \
  --out bench_format_ablation/delta_minimal_cmd_k32 --base-port 9960 2>&1
echo "============================================================"
echo "DELTA(K=32) A/B COMPLETE."
echo "  K=1  baseline : bench_format_ablation/compact_minimal_cmd_v3"
echo "  delta K=32 (new)    : bench_format_ablation/delta_minimal_cmd_k32"
