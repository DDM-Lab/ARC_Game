...
public class WorkforceIndicator : MonoBehaviour
{
    [Header("Indicator Sprites")]
    public Image[] indicatorSprites = new Image[4]; // four indicator UI elements
    ...
    public void UpdateIndicator(int trainedWorkers, int untrainedWorkers)
    {
        ...
        for (int i = 0; i < indicatorSprites.Length; i++)
        {
            ...
            {
                UpdateSingleIndicator(indicatorSprites[i], i < totalWorkforce, 
    ...
    /// </summary>
    void UpdateSingleIndicator(Image indicator, bool isActive, bool isTrained)
    {
