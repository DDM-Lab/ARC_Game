#!/usr/bin/env bash
# K=16 ablation cell: the winning compact-cmd format, now HISTORY-CARRYING (16-turn window).
# Identical code path to bench_format_ablation/compact_minimal_cmd_v3 (needStaff block + v3 prompts
# + compact obs) — the ONLY difference is --history 16: the policy sees an append-only window of
...
echo "### K=16 A/B  compact cmd + --history 16  -> bench_format_ablation/compact_minimal_cmd_k16"
python3 benchmark_models.py --policy llm --models "$MODELS" \
  --image_mode none --action_format cmd --transfers manual \
  --system_prompt minimal --obs_encoding compact --history 16 --reasoning_effort low \
  --episodes 3 --rounds 32 --workers 8 \
  --out bench_format_ablation/compact_minimal_cmd_k16 --base-port 9940 2>&1
echo "============================================================"
echo "K=16 A/B COMPLETE."
echo "  K=1  baseline : bench_format_ablation/compact_minimal_cmd_v3"
echo "  K=16 (new)    : bench_format_ablation/compact_minimal_cmd_k16"
